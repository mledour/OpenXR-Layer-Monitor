...
# template don't collide. Two products sharing an AppId make the
# second installer behave as an upgrade of the first, planting your
# DLL in the other layer's install dir. uppercase to match the
# existing {{XXXX-XXXX-...}} convention in installer.iss.
#
# Prefer `uuidgen` (BSD / macOS / Linux util-linux), fall back to
# `python -c 'import uuid; print(uuid.uuid4())'` if missing.
if command -v uuidgen >/dev/null 2>&1; then
    installer_app_id="$(uuidgen | tr '[:lower:]' '[:upper:]')"
elif command -v python3 >/dev/null 2>&1; then
    installer_app_id="$(python3 -c 'import uuid; print(str(uuid.uuid4()).upper())')"
else
    echo "error: need either uuidgen or python3 to generate Installer AppId" >&2
    exit 1
fi
