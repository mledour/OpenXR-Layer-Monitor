...
Then open `XR_APILAYER_MLEDOUR_monitor.sln` (real names
...
## Logging from the frame thread — the DBWinMutex trap

OpenXR layers run *inside* the host application's process. The OpenXR
runtime and its compositor typically live in a **separate** process,
and many of them — Pimax OpenXR being a confirmed case, SteamVR
suspected, others likely — call `OutputDebugStringA` for their own
diagnostics. `Log()` / `ErrorLog()` / `DebugLog()` in this template
end up calling the same Win32 API.

`OutputDebugStringA` takes a kernel-wide named mutex (`DBWinMutex`)
to serialise writes to the shared `DBWIN_BUFFER` section. Contention
on that mutex is **inter-process**: a layer that logs at 1 Hz from
the frame thread will block on the compositor's own log calls (and
vice versa). A few-hundred-microsecond stall on the frame thread,
landing between `xrEndFrame` returning and the next `xrWaitFrame`,
is enough to push the compositor outside its frame-pacing window —
observed empirically on Pimax OpenXR 0.1.0 as **zero pixels reaching
the HMD** even though the OpenXR session transitioned to VISIBLE
normally. No error code, no crash; the host just renders to a HMD
that stays black until the layer is uninstalled.

The trap is that the per-call cost looks negligible (a few µs in
steady state, per `log.h`) so 1 Hz feels safe. It is not. The cost
the doc warns about is *intra-process* contention with an attached
debugger; the inter-process DBWinMutex contention is silent and only
manifests as a visual symptom on the HMD.

Rules:

| Cadence | Mechanism |
|---|---|
| **Per-frame telemetry** (FPS, frametimes, headroom, GPU time…) | `TraceLoggingWrite` (ETW). Lock-free, ~50 ns per event when no consumer is attached, no kernel transition, no inter-process locks. Capture with `scripts\Tracing.wprp` + WPA / PerfView. |
| **One-shot diagnostics** ("first time we see X") | `Log()` guarded by a `m_xxxLogged` one-shot flag. See `m_fovLogged` / `m_strippedAppLayerLogged` in `XR_APILAYER_MLEDOUR_fov_crop/layer.cpp` for the canonical pattern. Re-arm to `false` in `xrCreateSession` if the event is session-scoped. |
| **Init / shutdown / session-scoped** | `Log()` unguarded. Called rarely enough that DBWinMutex contention is invisible. |
| **Periodic `Log()` from `xrWaitFrame` / `xrBeginFrame` / `xrEndFrame` / `xrLocateViews`**, even at 0.1 Hz | **Never.** Use ETW. The "is my layer alive?" sanity-light pattern is what kills the HMD. |

Bisection record that established this rule, on
`XR_APILAYER_MLEDOUR_xr_telemetry` PR #1:

```
full impl (math + ETW + 1 Hz Log)        : HMD black
pass-through + first-call ErrorLog       : HMD renders
math + atomics, no ETW, no Log           : HMD renders
math + ETW, no Log                       : HMD renders
final = math + ETW, Log removed          : HMD renders
```

`TraceLoggingWrite` at every frame was *not* the problem; the once-per-90-frame
`Log()` summary was.

## Singleton lifecycle — don't break it

The framework's auto-generated `xrDestroyInstance`
(`dispatch.gen.{h,cpp}`) calls `ResetInstance()`, which does
`g_instance.reset()` on the global `std::unique_ptr<OpenXrApi> g_instance`
declared in `dispatch.gen.h`. The template's `GetInstance()` in
`layer.cpp` writes to that same `g_instance`:
...
This gives each `XrInstance` a **fresh** `OpenXrLayer` whose destructor
runs at `xrDestroyInstance` time — application threads alive, mutexes
sane, RAII shutdown of stateful members (threads, files, GPU handles)
works as expected. This is the upstream `OpenXR-Layer-Template`
behaviour; keep it.

### The static-singleton anti-pattern — don't introduce it

Replacing `GetInstance()` with a function-local static unique_ptr breaks
...
1. `g_instance` is never written, so `ResetInstance()` becomes a no-op.
2. The `OpenXrLayer` singleton lives until process exit, surviving
   every `xrCreateInstance` / `xrDestroyInstance` cycle.
3. The destructor runs at static-destruction time, **after**
   `ExitProcess()` has terminated every other thread — including any
   helper threads your layer spawned. Their mutexes are orphaned and
   blocking `join()` / lock acquisition deadlocks the process.
4. Apps and shims that create multiple `XrInstance` objects in sequence
   (probe → real) now hit the **same** `OpenXrLayer` twice. Any state
   your `xrCreateInstance` override allocated the first time is still
   live when the second call arrives.

OpenComposite and OXR Toolkit both use the probe-then-real flow:

1. A "probe" `XrInstance` (often 0 extensions) to enumerate capabilities.
2. `xrDestroyInstance` on the probe.
3. The "real" `XrInstance` with the actually-requested extensions.

With the upstream pattern, each gets its own `OpenXrLayer`. With the
static-unique_ptr anti-pattern, they share one — and the second
init becomes a minefield. We hit this on
`XR_APILAYER_MLEDOUR_xr_telemetry` PR #1: a second
`m_thread = std::thread(...)` on a joinable thread per
[thread.thread.assign] calls `std::terminate()` — process dies with no
XR error, no exception for the framework's try/catch, no WER dump.
DR2 + OpenComposite and LMU + OXR Toolkit both crashed silently
before the underlying `GetInstance()` was fixed.

### If you spawn helper threads from xrCreateInstance

Even with the correct `g_instance` lifecycle, a few good habits:

| Stateful member | Watch for | Good practice |
|---|---|---|
| `std::thread m_thread;` | `~std::thread` calls `std::terminate()` if joinable; reassignment to joinable does too | RAII: own the thread by an object whose destructor `join()`s; never `m_thread = std::thread(...)` unconditionally |
| `std::mutex` / `condition_variable` | Held when the owning thread is killed → orphaned, future acquirers deadlock | At process-exit, **don't** trust `std::lock_guard` to complete. Use `std::timed_mutex` + bounded `try_lock_for`, fall back to `detach()` on timeout |
| `std::ofstream` | Buffered writes lost on crash; `is_open()` true does not mean writeable | Periodic `flush()`; check `good()` after each batch, log once on failure |
| Win32 handles (`HANDLE`, `IDXGISwapChain*`, …) | RAII via `wil::unique_handle` / `Microsoft::WRL::ComPtr` | Don't write naked `CloseHandle` / `Release` ladders |

### History

- Upstream `OpenXR-Layer-Template` (mbucchia): correct `g_instance`
  pattern.
- `XR_APILAYER_MLEDOUR_fov_crop`: correct pattern (matches upstream).
- This template (`OpenXR-Layer-Template-Plus`): regressed to the
  static-unique_ptr anti-pattern at some unknown earlier point.
  Restored to the upstream pattern alongside this document update.
- `XR_APILAYER_MLEDOUR_xr_telemetry` PR #1: inherited the broken
  pattern from this template, hit the issue on DR2 + OpenComposite
  and LMU + OXR Toolkit, then fixed both `GetInstance()` and added
  defence-in-depth (idempotent CsvWriter, `timed_mutex` +
  `WaitForSingleObject(100ms)` in shutdown) to catch the
  anti-pattern if a future refactor reintroduces it.
...
- `XR_APILAYER_MLEDOUR_monitor-<version>-x64-Setup.exe` —
  Inno Setup installer (recommended for end users)
- `XR_APILAYER_MLEDOUR_monitor-Release-x64.zip` — raw DLL +
  JSON + PowerShell scripts, for manual installation or development
- `XR_APILAYER_MLEDOUR_monitor-Debug-x64.zip` — debug build
